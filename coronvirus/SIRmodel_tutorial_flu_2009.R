...
parameters_values <- c(
  beta  = (1.3/7)/N, # infectious contact rate (/person/day)
...
library(ggplot2)
ggplot(aes(x = date, y = y, colour = sir), data = subset(dat, sir %in% c("I"))) + geom_line() + geom_vline(aes(xintercept = as.Date("2020-03-14"))) 



library(ggplot2)
ggplot(aes(x = date, y = y, colour = sir), data = subset(dat, sir %in% c("I"))) + geom_line() + geom_vline(aes(xintercept = as.Date("2020-03-14"))) + geom_hline(aes(yintercept = 924000))
